...
@Singleton
class ChatRtdbRepository
@Inject
constructor(private val rtdb: FirebaseDatabase) :
    ChatRepository {
    private fun chatRef(roomId: String): DatabaseReference =
        rtdb.getReference("chats").child(roomId)

    override fun observeRoomChat(roomId: String, limit: Int): Flow<List<ChatMessage>> =
        callbackFlow {
            ...
            val listener =
                object : ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        val list =
                            snapshot.children.mapNotNull { s ->
                                val id = s.key ?: return@mapNotNull null
                                val user =
                                    s.child("user").getValue(String::class.java)
