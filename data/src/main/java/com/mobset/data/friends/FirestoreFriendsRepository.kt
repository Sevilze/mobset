...
constructor(
    ...
    private fun reqIncomingCol(uid: String) =
        db.collection("friends").document(uid).collection("requests_incoming")

    private fun reqOutgoingCol(uid: String) =
        db.collection("friends").document(uid).collection("requests_outgoing")

    private fun roomInvitesCol(uid: String) =
        db.collection("invites").document(uid).collection("room")
    ...
    override fun observeFriends(uid: String): Flow<List<Friend>> = callbackFlow {
        val reg =
            friendsCol(uid).addSnapshotListener { snap, _ ->
                val list =
    ...
    override fun observeIncomingRequests(uid: String): Flow<List<FriendRequest>> = callbackFlow {
        val reg =
            reqIncomingCol(uid).addSnapshotListener { snap, _ ->
                val list =
                    snap?.documents?.map { d ->
                        FriendRequest(
                            id = d.id,
                            fromUid = d.getString("fromUid") ?: d.id,
                            at = d.millis("at")
                        )
                    } ?: emptyList()
                trySend(list)
    ...
    override fun observeRoomInvites(uid: String): Flow<List<RoomInvite>> = callbackFlow {
        val reg =
            roomInvitesCol(uid).addSnapshotListener { snap, _ ->
                val list =
                    snap?.documents?.mapNotNull { d ->
    ...
    override suspend fun sendRoomInvite(fromUid: String, toUid: String, roomId: String) {
        ...
        require(roomState?.status?.name == "WAITING") {
            "Can only invite while the room is in WAITING state"
        }

        roomInvitesCol(toUid)
            .add(
                mapOf(
                    "fromUid" to fromUid,
                    "roomId" to roomId,
