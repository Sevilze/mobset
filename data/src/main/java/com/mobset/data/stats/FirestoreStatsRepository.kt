...
@Singleton
class FirestoreStatsRepository @Inject constructor(
    ...
    override suspend fun incrementGamesPlayed(uid: String) {
        doc(uid).set(mapOf("gamesPlayed" to FieldValue.increment(1)), com.google.firebase.firestore.SetOptions.merge()).await()
    }

    override suspend fun recordBestTime(uid: String, timeMs: Long) {
        doc(uid).set(mapOf("bestTimeMs" to timeMs), com.google.firebase.firestore.SetOptions.merge()).await()
    }

    override suspend fun addSetsFound(uid: String, count: Int) {
        doc(uid).set(mapOf("setsFound" to FieldValue.increment(count.toLong())), com.google.firebase.firestore.SetOptions.merge()).await()
    }
}
