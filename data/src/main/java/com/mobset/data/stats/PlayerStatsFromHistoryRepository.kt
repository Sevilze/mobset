 ...
 */
@Singleton
class PlayerStatsFromHistoryRepository
@Inject
constructor(
    ...
    override fun observeAggregatedStats(
        ...
        history.observeUserGames(playerId, gameMode, playerMode).map { games ->
            if (games.isEmpty()) {
                return@map AggregatedPlayerStats(
                    playerId = playerId,
                    filterMode = gameMode,
            ...
            val avgLen = if (finishedGames == 0) null else sumDurations / finishedGames
